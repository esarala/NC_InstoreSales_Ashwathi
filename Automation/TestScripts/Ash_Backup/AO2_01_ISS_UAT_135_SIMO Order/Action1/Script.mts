...
'Designed By Date:Ashwathi
'Designed Date(MM/DD/YY): 10/05/2021
...
TestScriptName = "AO2_01_ISS_UAT_135"
...
If JavaWindow("Oracle Retail Xstore Point").JavaButton("Confirm").Exist(2) Then
 ...
 If (JavaWindow("Oracle Retail Xstore Point").JavaButton("Confirm").Exist(2)) Then
	Reporter.ReportEvent micPass, "SIMO Order - Transaction Completed","Test Passed"
	Else
	Reporter.ReportEvent micFail, "SIMO Order - Transaction not completed","Test Failed"
End if
...
If JavaWindow("text:=Oracle Retail.*").JavaStaticText("text:=£.*").Exist(2) Then
   ...
   Wait 2
    JavaWindow("Oracle Retail Xstore Point").JavaButton("CASH amount").Set CashAmount
    Wait 2
    JavaWindow("Oracle Retail Xstore Point").JavaButton("CASH amount").Click
 End If
wait 2
